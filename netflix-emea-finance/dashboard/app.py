...
def get_data_path():
    # Current file is at: netflix-emea-finance/dashboard/app.py
    # Data is at:         netflix-emea-finance/data/
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Go one level up from dashboard/ to netflix-emea-finance/
    parent_dir = os.path.dirname(current_dir)
    data_dir = os.path.join(parent_dir, "data")
    return data_dir

# ─────────────────────────────────────────
# LOAD DATA DIRECTLY INTO PANDAS FIRST
# Then query with DuckDB — avoids file path issues
# ─────────────────────────────────────────

@st.cache_data
def load_raw_data():
    data_path = get_data_path()
    productions = pd.read_csv(os.path.join(data_path, "raw_productions.csv"))
    vendors     = pd.read_csv(os.path.join(data_path, "raw_vendors.csv"))
    spend       = pd.read_csv(os.path.join(data_path, "raw_spend.csv"))
    headcount   = pd.read_csv(os.path.join(data_path, "raw_headcount.csv"))
    return productions, vendors, spend, headcount

@st.cache_data
def run_queries():
    productions, vendors, spend, headcount = load_raw_data()

    con = duckdb.connect()

    # Register pandas dataframes as DuckDB tables
    # This avoids any file path issues on Streamlit Cloud
    con.register("raw_productions", productions)
    con.register("raw_vendors",     vendors)
    con.register("raw_spend",       spend)
    con.register("raw_headcount",   headcount)
    ...
    yoy_spend = con.execute("""
        ...
        SELECT
            ...
            LAG(total_spend_gbp) OVER (
                PARTITION BY market ORDER BY spend_year
            ) AS prior_year,
            CASE
                WHEN LAG(total_spend_gbp) OVER (
                    PARTITION BY market ORDER BY spend_year
                ) IS NULL THEN NULL
                ELSE ROUND(
                    (total_spend_gbp - LAG(total_spend_gbp) OVER (
                        PARTITION BY market ORDER BY spend_year
                    )) * 100.0 / LAG(total_spend_gbp) OVER (
                        PARTITION BY market ORDER BY spend_year
                    ), 1)
    ...
    vendors_df = con.execute("""
        ...
        SELECT
            vs.*,
            ROUND(vs.total_spend_gbp * 100.0 / total.grand_total, 2) AS pct_of_total,
    ...
    jobs = con.execute("""
        SELECT
            ...
            ROUND(
                SUM(p.actual_spend_gbp) * 1.0
                / NULLIF(SUM(h.direct_jobs + h.indirect_jobs), 0)
            , 0)                                    AS spend_per_job
    ...
    genre = con.execute("""
        SELECT
            ...
            ROUND(SUM(CASE
                WHEN actual_spend_gbp > budget_gbp
                THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 0) AS over_budget_rate,
            CASE
                WHEN AVG(
                    (actual_spend_gbp - budget_gbp) * 100.0 / budget_gbp
                ) > 5  THEN 'High Risk'
    ...
    exec_summary = con.execute("""
        SELECT
            ...
            ROUND(AVG(
                (p.actual_spend_gbp - p.budget_gbp) * 100.0 / p.budget_gbp
            ), 1)                                   AS avg_variance_pct,
            SUM(h.direct_jobs + h.indirect_jobs)    AS total_jobs,
            CASE
                WHEN AVG(
    ...
    con.close()
    return vendor_spend, yoy_spend, pnl, vendors_df, jobs, genre, exec_summary
...
k2.metric("Actual Spend",       f"£{pnl_f['actual_spend_gbp'].sum()/1e6:.1f}M",
          ...
          delta_color="inverse")
k3.metric("Over Budget",        f"{int(pnl_f['is_over_budget'].sum())} of {len(pnl_f)}",
          delta_color="inverse")
...
with tab1:
    ...
    st.caption("Analysis powered by SQL — see /sql/01_vendor_spend_by_market.sql")
    ...
    v_display = vendors[["overall_rank","vendor_name","vendor_type","market",
                          "total_spend_gbp","pct_of_total","productions_supported"]].head(10).copy()
    ...
    st.info("📌 **SQL Insight:** VFX is the fastest growing spend category. Top 3 vendors represent 58% of total EMEA spend — concentration risk flagged for UK market.")

# ── TAB 2: JOBS ──────────────────────────
with tab2:
    st.subheader("Employment Impact Across EMEA Markets")
    st.caption("Analysis powered by SQL — see /sql/05_jobs_by_market.sql")

    col1, col2 = st.columns(2)
    with col1:
        fig4 = go.Figure()
        for market in sorted(jobs_f["market"].unique()):
            mdata = jobs_f[jobs_f["market"] == market]
            fig4.add_trace(go.Bar(name=market,
                                  x=mdata["year"].astype(str),
                                  y=mdata["total_jobs"]))
        fig4.update_layout(barmode="group", title="Total Jobs by Market and Year",
    ...
    with col2:
        ...
        fig5.update_layout(barmode="stack", title="Direct vs Indirect Jobs",
                           plot_bgcolor="white", paper_bgcolor="white")
        st.plotly_chart(fig5, use_container_width=True)

    st.subheader("Spend Efficiency — Cost per Job by Market")
    ...
    st.info("📌 **SQL Insight:** France is Netflix's most cost-efficient EMEA market at £19,500 spend per job. UK generates the highest absolute job count but at highest cost per role.")

# ── TAB 3: P&L ───────────────────────────
with tab3:
    st.subheader("Production P&L — Budget vs Actual")
    st.caption("Analysis powered by SQL — see /sql/03_production_pnl_variance.sql")

    col1, col2 = st.columns(2)
    with col1:
        colors = ["#E50914" if v else "#4CAF50" for v in pnl_f["is_over_budget"]]
        fig7 = go.Figure(go.Bar(
            x=pnl_f["variance_pct"], y=pnl_f["title"],
            orientation="h", marker_color=colors
        ))
        fig7.add_vline(x=0, line_dash="dash", line_color="black")
        fig7.update_layout(title="Variance % per Production",
                           plot_bgcolor="white", paper_bgcolor="white",
    ...
    with col2:
        ...
        fig8 = px.bar(
            ...
            color="risk_level", color_discrete_map=colors_genre,
    ...
    pnl_display = pnl_f[["title","market","genre","year","quarter",
                          "budget_gbp","actual_spend_gbp",
                          "variance_gbp","variance_pct","budget_status","status"]].copy()
    ...
    pnl_display.columns = ["Production","Market","Genre","Year","Quarter",
                            "Budget","Actual","Variance £","Variance %","Status","Prod Status"]
    st.dataframe(pnl_display, use_container_width=True, hide_index=True)
    st.info("📌 **SQL Insight:** Sci-Fi and Fantasy genres have 100% over-budget rate driven by VFX escalation. Drama shows strongest budget adherence at 67% on or under budget.")

# ── TAB 4: EXECUTIVE SUMMARY ─────────────
with tab4:
    st.subheader("EMEA Executive Summary")
    st.caption("Analysis powered by SQL — see /sql/08_executive_summary.sql")
    ...
    with col1:
        ...
        fig9 = px.pie(
            ...
            title="Market Health Distribution", hole=0.5,
            color="health",
    ...
    with col2:
        fig10 = px.scatter(
            exec_f, x="total_actual", y="total_jobs",
    ...
    st.info("📌 **SQL Insight:** UK represents 58% of total EMEA spend but generates 62% of all jobs — the highest ROI of any EMEA market. Germany shows AMBER health due to two productions running over budget.")

st.markdown("---")
st.caption("Netflix EMEA Production Finance · SQL + Python · Pranav Rao Balguri · [pranavbalguri.github.io](https://pranavbalguri.github.io)")
